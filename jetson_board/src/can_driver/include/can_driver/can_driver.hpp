...
#define CAL_IMU_FRAME_LENGTH        9 // state (1 byte) + imu_calibration_samples_ (4 bytes) + imu_rate_ (4 bytes)
#define CAL_CAM_FRAME_LENGTH        9 // state (1 byte) + camera_calibration_samples_ (4 bytes) + camera_rate_ (4 bytes)
#define RUN_FRAME_LENGTH            1 // state (1 byte)
...
class can_driver : public rclcpp::Node {
    ...
    private:
        ...
        // Camera and IMU rates and number of samples used for calibration
        int32_t camera_rate_; // [FPS] User defined in YAML file
        int32_t camera_calibration_samples_ = 0; // User defined in YAML file
        int32_t imu_rate_; // [Hz] User defined in YAML file
        int32_t imu_calibration_samples_ = 20; //5*208;
        ...
        void camera_calibration_samples_callback(const std_msgs::msg::Int32::SharedPtr msg);
